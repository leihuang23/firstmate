...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, Omp uses its tracked `.omp/extensions/` pair, and OpenCode uses its TUI plugin.
...
For Omp secondmate launches, `fm-spawn.sh` starts `omp --auto-approve` with `-e` pointed at the secondmate home's own tracked `.omp/extensions/fm-primary-omp-watch.ts` and `.omp/extensions/fm-primary-turnend-guard.ts`.
