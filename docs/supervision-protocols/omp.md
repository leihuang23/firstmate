...
5. The extension starts `bin/fm-watch-arm.sh --restart`, keeps the child attached to the live omp process, starts the successor cycle before delivering an actionable wake, and records a rejected delivery in `state/.omp-watch-delivery-failed` for the next session-start digest.
