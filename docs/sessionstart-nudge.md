...
| Kimi | No project-local hook transport; `SessionStart` in `$KIMI_CODE_HOME/config.toml` is observation-only (kimi-code 0.26.0 docs). | Fail-open: the primary must still run `bin/fm-session-start.sh` once after opening a kimi session. |
