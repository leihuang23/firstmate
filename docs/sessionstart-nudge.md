...
Before printing, the wrapper reads `state/.lock` and walks at most eight parents from its own pid, matching `bin/fm-lock.sh` and the Pi and Omp extensions' `lockOwnership()` ancestry depth.
...
| Omp | `.omp/extensions/fm-primary-turnend-guard.ts` handles `session_start` (reasons `startup`, `new`, `resume`, plus the reason-less print-mode payload), then injects the wrapper output with `pi.sendMessage` and `display: false`. | The custom message reaches model context, and `.omp/extensions/` auto-loads with no trust gate. |
...
It also verifies every tracked transport registration listed above, including Pi, pi-signed, and Omp.
...
`tests/fm-turnend-guard.test.sh`, `tests/fm-pi-watch-extension.test.sh`, `tests/fm-omp-watch-extension.test.sh`, and `tests/fm-daemon.test.sh` cover marked guard, monitoring, and away-mode delivery.
