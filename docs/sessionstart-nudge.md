...
| Omp | `.omp/extensions/fm-primary-turnend-guard.ts` handles `session_start` (reasons `startup`, `new`, `resume`, plus the reason-less print-mode payload), then injects the wrapper output with `pi.sendMessage` and `display: false`. | Verified live 2026-07-26 on omp 17.1.3: the injected nudge reached the model and was obeyed; `.omp/extensions/` auto-loads with no trust gate. |
