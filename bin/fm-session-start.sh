...
if [ "$PRIMARY_HARNESS" = pi ] && [ "$READ_ONLY" -eq 0 ]; then
  ...
  if ! extension_loaded "$PI_WATCH_MARKER" "$PI_WATCH_VERSION" "$PI_LOCK" \
    || ! extension_loaded "$PI_TURNEND_MARKER" "$PI_TURNEND_VERSION" "$PI_LOCK"; then
